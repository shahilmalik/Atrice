...
#         print(f"Updating camid for {student.name}")
...
#     available_cameras = []
#     cap_list = []
#     print(f'The num cam is {num_cameras}')
#     for i in range(4):  # Assuming you're checking for up to 4 cameras
#         try:
...
#                 cap_list.append(cap)
#             else:
#                 cap.release()  # Release the camera if it couldn't be opened
#                 print(f"Camera {i} failed to properly initialize.")
#         except cv2.error as e:
#             print(f"OpenCV error while initializing camera {i}: {e}")
#         except Exception as e:
#             print(f"Error while initializing camera {i}: {e}")

#     if num_cameras > len(available_cameras):
#         return HttpResponse(f"Error: The specified number of cameras is {num_cameras}. but only {len(available_cameras)} available on this device")
...
#     # Release the cameras after the loop has finished
#     for cap_data in cap_dict.values():
#         cap = cap_data["cap"]
#         cap.release()
...
#     #     new_num_cameras = int(request.POST.get('num_cameras', num_cameras))
#     #     if new_num_cameras != num_cameras:
#     #     # Release excess camera streams
#     #         for cap in cap_list[new_num_cameras:]:
#     #             cap.release()
#     #         cap_list = cap_list[:new_num_cameras]
#     #         cap_dict = {cap: cap_dict[cap] for cap in cap_list}

#     #         num_cameras = new_num_cameras
#     #         for i in range(num_cameras - len(cap_list)):
#     #             cap = cv2.VideoCapture(i + len(cap_list))
#     #             if cap.isOpened():
#     #                 cap_list.append(cap)
#     #                 cap_dict[cap] = {"ret": None, "img": None}
#     #                 print(f"Camera {i + 1} opened successfully")
#     #             else:
#     #                 print(f"Error opening camera {i + 1}")
#         # cam1 = "CAM 1"
#         # cam2 = "CAM 2"
#         # cam3 = "CAM 3"
#         # cam4 = "CAM 4"
...
#     context = {
#         'students_json': students_json,
#         'form': form,  # Use the previously initialized form
#     }
...
#     image_size_mb = image.size / (1024 * 1024)  # Convert to MB
